...
export default function AdminReportsPage() {
  const reports = [
    { id: 1, title: "Reporte mensual de lugares", description: "Resumen de lugares turísticos agregados durante el mes.", date: "Mayo 2026", icon: "⌖" },
    { id: 2, title: "Reporte de pueblos activos", description: "Listado de pueblos turísticos disponibles en la plataforma.", date: "Mayo 2026", icon: "⌂" },
    { id: 3, title: "Reporte de usuarios registrados", description: "Cantidad de usuarios registrados y actividad general.", date: "Mayo 2026", icon: "♙" },
    { id: 4, title: "Reporte de códigos QR", description: "Resumen de códigos QR generados y escaneos simulados.", date: "Mayo 2026", icon: "▦" }
  ];

  const exportReport = (reportTitle = "reporte-general") => {
    const content = `Turismo Local POC\n${reportTitle}\n\nReporte generado correctamente.\nPueblos: 24\nLugares: 142\nUsuarios: 1284\nQR generados: 3450`;
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");

    link.href = url;
    link.download = `${reportTitle.toLowerCase().replaceAll(" ", "-")}.txt`;
    link.click();
  ...
  return (
    <AdminLayout>
      <div className="admin-page-header">
        ...
        <button className="btn-admin-primary" onClick={() => exportReport("reporte-general")}>
          ⇩ Exportar reporte
        </button>
      </div>
            ...
            <div className="col-md-6" key={report.id}>
              <div className="card border-0 shadow-sm rounded-4 p-4 h-100">
                ...
                <div className="d-flex justify-content-between align-items-center mt-3">
                  ...
                  <button className="btn btn-info text-white rounded-pill px-4" onClick={() => exportReport(report.title)}>
                    Ver reporte
                  </button>
                </div>
              </div>
